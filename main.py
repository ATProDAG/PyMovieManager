...
class FileRelated:
  ...
  def print_all(self):
    with open("Watched.csv","r") as file:
      print()
      print(colored(" Titles you have watched ".center(80,'+'),'red',attrs=['bold']))
      reader = csv.reader(file)
      next(reader)
      for row in reader:
        if row == []:
            ...
            print(f"{row[0]}({row[1]}) - {row[2].title()}".center(80))
    with open("ToWatch.csv","r") as file:
      print()
      print(colored(" Titles you want to watch ".center(80,'+'),'red',attrs=['bold']))
      reader = csv.reader(file)
      next(reader)
      for row in reader:
        if row == []:
...
class Imdbsearch:
  ...
  def search_on(self):
    ...
    if correct == "Y":
        ...
        print("\n")
        print(colored("Try A More Specific Search Term(s)".center(80,'*'),'yellow'))
...
def print_main():
    print(colored(" Movies/Series CSV Based Management System ".center(80,"-"),'blue',attrs=['bold','reverse']))
...
while(y_n == "y"):
  ...
  if choice == "a":
    ...
    if (toadd[0] in list_watched) or (toadd[0] in list_to_watch):
      print(colored("\nThe Movie/Series is already in Watchlist or Watch History".center(80),'red'))
    else:
      path_temp = int(input(colored("\nWhere to add? (1 for Watched, 2 for Watchlist): ",'magenta')))
      if path_temp == 1:
    ...
    target_top = int(input("Enter 1 for Top 250 Movies, 2 for Top 250 Series: "))
    if target_top == 1:
      ...
      print("\n")
      print(colored("Wrong Input".center(80, "*"),'red')) 
  else:
    print("\n")
    print(colored("Wrong Input".center(80,'*'),'red'))
    get_all_lists()

print(colored("Thanks for using! -- By Ansh Goel".center(80),'grey','on_yellow',attrs=['bold']))
